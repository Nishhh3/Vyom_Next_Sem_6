...
# ─────────────────────────────────────────────
# Seed data — same users across all banks
# (mirrors kyc_users phone numbers)
# ─────────────────────────────────────────────
SEED_USERS = [
...
def init_db(db_path: str, bank_name: str, ifsc_prefix: str):
    ...
    # Seed customers + accounts + transactions only if empty
    if cur.execute("SELECT COUNT(*) FROM customers").fetchone()[0] == 0:
        for user in SEED_USERS:
            ...
            # 1–2 accounts per user per bank
            ...
            for idx in range(num_accounts):
                ...
                # Seed 10–20 past transactions per account
                running_bal = balance
...
def make_bank_app(bank_name: str, db_path: str) -> Flask:
    ...
    # ── Initiate transfer (debit from account) ──
...
# ─────────────────────────────────────────────
# Vyom AA Gateway (port 7000) — aggregates all 3
# ─────────────────────────────────────────────
def make_gateway_app() -> Flask:
    ...
    # Fetch all accounts for a phone number across all 3 banks
    ...
    # Get balance from a specific bank
...
if __name__ == "__main__":
    ...
    # Start each bank on its port in a daemon thread
    threads = []
    ...
    # Gateway runs on main thread
    gw = make_gateway_app()
    print(f"  🌐 Vyom AA Gateway    → http://localhost:7000")
    print("\nEndpoints:")
    print("  GET  :7000/vyom/accounts?phone=9876543210        ← all banks for a phone")
    print("  GET  :7000/vyom/ICICI/accounts/<id>/balance")
    print("  GET  :7000/vyom/SBI/accounts/<id>/transactions")
    print("  POST :7000/vyom/HDFC/accounts/<id>/transfer")
    print("  GET  :7000/health")
    print("\nsqlite_web UI (run separately):")
    print("  sqlite_web data/icici.db -p 8081")
    print("  sqlite_web data/sbi.db   -p 8082")
    print("  sqlite_web data/hdfc.db  -p 8083")
